...
export const POSTGRES_PORT         = Math.max(~~(process.env.POSTGRES_PORT || ''), 0) || 5432
...
export const POSTGRES_IDLE_TIMEOUT = Math.max(~~(process.env.POSTGRES_IDLE_TIMEOUT || '') || 0, 0) || 10
// PGSQL server pool size, default 10
export const POSTGRES_POOL_SIZE    = Math.max(~~(process.env.POSTGRES_POOL_SIZE || '') || 0, 0) || 10
