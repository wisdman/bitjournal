...
export class Hash {
  static hashString(input: string | Uint8Array): string {
    const buffer = typeof input === 'string' ? encode(input) : input
    const hashBuffer = new Uint8Array(Hash.hash(buffer).buffer)
    return Array.prototype.map.call(hashBuffer, (x:number) => ('00' + x.toString(16)).slice(-2)).join('')
  }
