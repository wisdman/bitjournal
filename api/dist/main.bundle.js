...
class session_middleware_SessionMiddleware extends Middleware {
    constructor(BaseClass = Session, readonly = false) {
        super();
        this.BaseClass = BaseClass;
...
class otp_OTP {
    ...
    keyuri(user = 'User', service = 'Service') {
...
class auth_api_AuthAPI extends route_middleware_RouteMiddleware {
...
users_api___decorate([
    ...
    users_api___metadata("design:paramtypes", [context_Context, Function]),
    users_api___metadata("design:returntype", Promise)
], users_api_UsersAPI.prototype, "create", null);
users_api___decorate([
    Post('/users/:id'),
    ACL([
        UserRoleEnum.Administrator,
...
process.env.TZ = 'UTC';
new service_Service(new pgdb_middleware_PgDBMiddleware(), new session_middleware_UserSessionMiddleware(), ...Object.values(api_namespaceObject).map(api => new api()));
...
var Decoder = __webpack_require__(7).StringDecoder
...
class Pool extends EventEmitter {
  constructor (options, Client) {
    ...
    this.Client = this.options.Client || Client || __webpack_require__(8).Client
