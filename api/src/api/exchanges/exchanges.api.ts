...
const DATATABLE = 'exchanges'

export class ExchangesAPI extends RouteMiddleware {
  ...
  async getAll(ctx: Context, next: INext) {
    ...
    const query = new Query(DATATABLE).select(Exchange.MainFields)
    ...
    const resultItems = result.rows.map( item => new Exchange(item) )
  ...
  async getOne(ctx: Context, next: INext) {
    ...
    const resultItem = new Exchange(result.rows[0])
