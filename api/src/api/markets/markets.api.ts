...
const DATATABLE = 'markets'

export class MarketsAPI extends RouteMiddleware {
  ...
  async getAll(ctx: Context, next: INext) {
    ...
    const query = new Query(DATATABLE).select(Market.MainFields)
    ...
    const resultItems = result.rows.map( item => new Market(item) )
