...
export class StatusesAPI extends RouteMiddleware {
  ...
  async getAll(ctx: Context, next: INext) {
    ...
    const resultItems = result.rows.map( item => new Status(item) )
  ...
  async getOne(ctx: Context, next: INext) {
    ...
    const resultItem = new Status(result.rows[0])
