...
export class VideoAPI extends RouteMiddleware {
  ...
  async getAll(ctx: Context, next: INext) {
    ...
    const query = new Query(DATATABLE).select(Video.MainFields)
    ...
    const resultItems = result.rows.map( item => new Video(item) )
  ...
  async getOne(ctx: Context, next: INext) {
    ...
    const resultItem = new Video(result.rows[0])
