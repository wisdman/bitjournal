...
import { IMarket, IRating } from '@common/models'
...
export class MarketItemComponent {
  ...
  save() {
    ...
    this._apiService
        .post<IMarket>(postURL, this.itemForm.value)
  ...
  delete() {
    ...
    this._dialog.open({
      title: 'Удалить биржу?',
      message: `Вы уверены, что хотите удалить биржу "${this.itemForm.value.title}"?`,
      buttons: {
        'Отмена': false,
        'Удалить': true
      }
      ...
      if (result === true)
        this._apiService.delete<IMarket>(`/${API_BASE}/${this._id}`)
                        .subscribe( _ => this._location.back() )
    })
  }
}
