...
export class VideoListComponent implements OnInit, AfterViewInit {
  ...
  ngOnInit(){
    this._apiService
        .get< Array<IVideo> >(`${VIDEO_API_PATH}`)
  ...
  add() {
    this._dialog.request({
      ...
      this._apiService
          .post<IVideo>(`${VIDEO_API_PATH}`, { id, enable: true })
  ...
  delete(deletedItem: IVideoListItem) {
    this._dialog.open({
      ...
      this._apiService
        .delete<IVideo>(`${VIDEO_API_PATH}/${deletedItem.id}`)
  ...
  toggle(item: IVideoListItem) {
    this._apiService
        .post<{ enable: boolean }>(`${VIDEO_API_PATH}/${item.id}`, { enable: !item.enable })
